class Application {
    ...
    createChairs() {
        // Posições Z centrais de cada monitor (tela.002, tela.001, tela.003)
        const monitorZPositions = [-3.66, -5.77, -7.79];
        // Cadeiras ficam a X ≈ 21.5, viradas para +X (de frente para a mesa em X ≈ 22.5)
        const chairX = 21.5;
        // Chão do prédio ≈ 0.029
        const floorY = 0.029;

        const green      = [0.15, 0.55, 0.15]; // verde principal
        const darkGreen  = [0.10, 0.40, 0.10]; // verde escuro para pernas/estrutura

        for (const monitorZ of monitorZPositions) {
            // ── Assento ──
            // cubo unitário escalado: largura(Z)=0.7, altura(Y)=0.08, profundidade(X)=0.6
            const seatGeom = this.createGreenCubeGeometry(...green);
            const seatY = floorY + 0.50; // centro do assento
            this.renderer.addObject(
                seatGeom,
                new Vector3(chairX, seatY, monitorZ),
                new Vector3(0, 0, 0),
                new Vector3(0.6, 0.08, 0.7)
            );

            // ── Encosto (backrest) ── no lado -X do assento (costas da cadeira)
            const backGeom = this.createGreenCubeGeometry(...green);
            const backX = chairX - 0.27; // borda traseira do assento
            const backY = floorY + 0.78; // centro do encosto
            this.renderer.addObject(
                backGeom,
                new Vector3(backX, backY, monitorZ),
                new Vector3(0, 0, 0),
                new Vector3(0.08, 0.50, 0.7)
            );

            // ── Braço esquerdo (armrest) ──
            const armLGeom = this.createGreenCubeGeometry(...green);
            const armY = floorY + 0.60;
            this.renderer.addObject(
                armLGeom,
                new Vector3(chairX, armY, monitorZ + 0.32),
                new Vector3(0, 0, 0),
                new Vector3(0.6, 0.06, 0.06)
            );
            ...
            for (const [lx, lz] of legOffsets) {
                ...
                this.renderer.addObject(
                    legGeom,
                    new Vector3(lx, legY, lz),
                    new Vector3(0, 0, 0),
                    new Vector3(0.06, legH, 0.06)
                );
            }
        }
