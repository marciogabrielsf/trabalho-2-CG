class Application {
    ...
    async setupScene() {
        ...
        // ── Mobília dentro da sala no final do prédio (X≈22) ──

        // Mesa: cubo achatado (largo em X e Z, baixo em Y)
        const deskInRoom = this.renderer.addObject(
            cubeGeometry,
            new Vector3(22, 1.0, 0),   // centro da sala, Y=1 (altura da mesa)
            new Vector3(0, 0, 0),
            new Vector3(3.0, 0.15, 1.5) // 3m x 0.15m x 1.5m
        );

        // Pernas da mesa (4 cubos finos)
        const legScale = new Vector3(0.1, 0.85, 0.1);
        const legY = 0.425; // metade da altura da perna
        const legOffsets = [
            new Vector3(21.0, legY,  0.55),
            new Vector3(21.0, legY, -0.55),
            new Vector3(23.0, legY,  0.55),
            new Vector3(23.0, legY, -0.55)
        ];
        for (const legPos of legOffsets) {
            this.renderer.addObject(cubeGeometry, legPos, new Vector3(0,0,0), legScale);
        }

        // Monitor modelado no Blender (monitor.obj) em cima da mesa
        try {
            ...
            const monitor = this.renderer.addObject(
                monitorGeometry,
                new Vector3(22, 0.1, 0),  // em cima da mesa, última sala (X≈22)
                new Vector3(0, 0, 0),
                new Vector3(1, 1, 1)
            );
            console.log('Monitor loaded in last room');
        } catch (error) {
    ...
    async loadOBJModels() {
        ...
        // --- Mesa (desk) na última sala à direita ---
        const deskGeometry = Cube.createGeometry();
        // Sobrescrever cores para marrom (madeira)
        const brownColor = [0.55, 0.35, 0.17];
        for (let i = 0; i < deskGeometry.colors.length; i += 3) {
            deskGeometry.colors[i]     = brownColor[0];
            deskGeometry.colors[i + 1] = brownColor[1];
            deskGeometry.colors[i + 2] = brownColor[2];
        }
        const desk = this.renderer.addObject(
            deskGeometry,
            new Vector3(23, 0.43, 0),   // centro da sala mais à direita, sobre o chão
            new Vector3(0, 0, 0),
            new Vector3(1.8, 0.8, 1.0)  // mesa larga, baixa e com profundidade
        );
        this.objModels.push(desk);

        // --- Monitor em cima da mesa ---
        try {
            const monitorGeometry = await OBJLoader.load('./assets/models/monitor.obj', true);
            ...
            if (monitorGeometry.materials) {
                console.log('Monitor materials loaded:', monitorGeometry.materials.size);
                OBJLoader.applyMaterialColors(monitorGeometry, monitorGeometry.materials);
            }

            // Escala 0.15 → monitor pequeno. Base do monitor em y≈0.34*0.15=0.051
            // Topo da mesa em y = 0.43 + 0.4 = 0.83
            // Posição Y do monitor = 0.83 - 0.051 ≈ 0.78
            const monitor = this.renderer.addObject(
                monitorGeometry,
                new Vector3(23, 0.78, 0),
                new Vector3(0, 0, 0),
