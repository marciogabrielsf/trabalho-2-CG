class Application {
    ...
    async loadOBJModels() {
        ...
        // --- 3 Poltronas verdes viradas para a mesa, uma por monitor ---
        this.createChairs();

        console.log(`Loaded ${this.objModels.length} OBJ models successfully`);
    }

    createGreenCubeGeometry(r, g, b) {
        const geom = Cube.createGeometry();
    ...
    createChairs() {
        // Sala: parede interna x≈20.97, parede fundo x≈24.92
        // Mesa: x 22.54–24.50, superfície Y≈1.50, z -2.60 a -9.03
        // Monitores (centros Z): tela.002≈-3.66, tela.001≈-5.77, tela.003≈-7.79
        // Chão do prédio: Y ≈ 0.111

        const monitorZPositions = [-3.66, -5.77, -7.79];
        // Cadeiras entre a parede interna (x≈20.97) e a mesa (x≈22.54)
        const chairX = 21.75;
        const floorY = 0.111;

        const green     = [0.15, 0.55, 0.15];
        const darkGreen = [0.10, 0.40, 0.10];

        // Proporções da cadeira relativas à mesa (superfície Y≈1.50)
        const legHeight  = 0.85;                      // pernas altas para alcançar a mesa
        const seatThick  = 0.08;
        const seatTopY   = floorY + legHeight + seatThick; // ≈ 1.04
        const seatCenterY = seatTopY - seatThick / 2;      // ≈ 1.0

        for (const monitorZ of monitorZPositions) {
            // ── Assento ──
            const seatGeom = this.createGreenCubeGeometry(...green);
            this.renderer.addObject(
                seatGeom,
                new Vector3(chairX, seatCenterY, monitorZ),
                new Vector3(0, 0, 0),
                new Vector3(0.7, seatThick, 0.8)
            );

            // ── Encosto (backrest) ── costas viradas para -X (longe da mesa)
            const backGeom = this.createGreenCubeGeometry(...green);
            const backX = chairX - 0.30;
            const backH = 0.65;
            const backCenterY = seatTopY + backH / 2;
            this.renderer.addObject(
                backGeom,
                new Vector3(backX, backCenterY, monitorZ),
                new Vector3(0, 0, 0),
                new Vector3(0.08, backH, 0.8)
            );

            // ── Braço esquerdo (armrest) ──
            const armLGeom = this.createGreenCubeGeometry(...green);
            const armY = seatTopY + 0.15;
            this.renderer.addObject(
                armLGeom,
                new Vector3(chairX, armY, monitorZ + 0.37),
                new Vector3(0, 0, 0),
                new Vector3(0.7, 0.06, 0.06)
            );

            // ── Braço direito (armrest) ──
            const armRGeom = this.createGreenCubeGeometry(...green);
            this.renderer.addObject(
                armRGeom,
                new Vector3(chairX, armY, monitorZ - 0.37),
                new Vector3(0, 0, 0),
            ...
            // ── 4 Pernas ──
            const legCenterY = floorY + legHeight / 2;
            const legOffsets = [
                [chairX + 0.28, monitorZ + 0.32],
                [chairX + 0.28, monitorZ - 0.32],
                [chairX - 0.28, monitorZ + 0.32],
                [chairX - 0.28, monitorZ - 0.32]
            ];
            for (const [lx, lz] of legOffsets) {
                const legGeom = this.createGreenCubeGeometry(...darkGreen);
                this.renderer.addObject(
                    legGeom,
                    new Vector3(lx, legCenterY, lz),
                    new Vector3(0, 0, 0),
                    new Vector3(0.06, legHeight, 0.06)
                );
            }
        }

        console.log('3 green armchairs created facing monitors');
    }
