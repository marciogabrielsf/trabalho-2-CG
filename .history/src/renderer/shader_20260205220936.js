class ShaderProgram {
    ...
    static createFragmentShader() {
        return `
            ...
            // Luz pontual próxima (monitores / laboratório)
            uniform vec3 uPointLightPosition;
            uniform vec3 uPointLightColor;
            uniform float uPointLightIntensity;
            uniform int uEnablePointLight;

            // Controle de emissividade por objeto
            ...
            void main() {
                ...
                // Em superfícies emissivas, ignoramos sombras na luz principal
                float shadowFactor = (uIsEmissive == 1) ? 0.0 : shadow;
                vec3 phongMain = ambient + (1.0 - shadowFactor) * (diffuse + specular);

                // Contribuição da luz pontual próxima (sem sombras)
                vec3 pointContribution = vec3(0.0);
                if(uEnablePointLight == 1) {
                    vec3 pointDir = normalize(uPointLightPosition - vFragPos);
                    float pointDist = length(uPointLightPosition - vFragPos);
                    float attenuation = 1.0 / (1.0 + 0.09 * pointDist + 0.032 * pointDist * pointDist);
                    
                    float diffP = max(dot(norm, pointDir), 0.0);
                    vec3 diffuseP = diffP * uPointLightColor;
                    
                    vec3 reflectDirP = reflect(-pointDir, norm);
                    float specP = pow(max(dot(viewDir, reflectDirP), 0.0), 32.0);
                    vec3 specularP = 0.5 * specP * uPointLightColor;
                    
                    pointContribution = (diffuseP + specularP) * attenuation * uPointLightIntensity;
                }

                // Emissão própria (monitores): adiciona luz independente da posição da luz
                vec3 emission = (uIsEmissive == 1) ? (baseColor * uEmissiveStrength) : vec3(0.0);
                
                vec3 lighting = phongMain + pointContribution;
                vec3 result = lighting * baseColor + emission;
    ...
    static compile(gl) {
        ...
        const programInfo = {
            ...
            uniformLocations: {
                ...
                pointLightIntensity: gl.getUniformLocation(program, 'uPointLightIntensity'),
                enablePointLight: gl.getUniformLocation(program, 'uEnablePointLight'),
