class Renderer {
    constructor(gl) {
        ...
        // Luz pontual próxima (laboratório de informática / monitores)
        this.pointLightPosition = new Vector3(-5, 1.5, 0);
        this.pointLightColor = new Vector3(0.3, 0.6, 1.0);
        this.pointLightIntensity = 2.0;
    ...
    renderScene(camera) {
        ...
        // Configuração da luz pontual próxima (monitores)
        if (program.uniformLocations.pointLightPosition) {
            gl.uniform3f(
                program.uniformLocations.pointLightPosition,
                this.pointLightPosition.x,
                this.pointLightPosition.y,
                this.pointLightPosition.z
            );
        }
        if (program.uniformLocations.pointLightColor) {
            gl.uniform3f(
                program.uniformLocations.pointLightColor,
                this.pointLightColor.x,
                this.pointLightColor.y,
                this.pointLightColor.z
            );
        }
        if (program.uniformLocations.pointLightIntensity) {
            gl.uniform1f(
                program.uniformLocations.pointLightIntensity,
                this.pointLightIntensity
            );
        }
        if (program.uniformLocations.enablePointLight) {
            gl.uniform1i(
                program.uniformLocations.enablePointLight,
                this.enablePointLight ? 1 : 0
            );
        }
    ...
    renderObject(object, program) {
        ...
        // Configuração de material emissivo (monitores)
        if (program.uniformLocations.isEmissive) {
            gl.uniform1i(
                program.uniformLocations.isEmissive,
                object.isEmissive ? 1 : 0
            );
        }
        if (program.uniformLocations.emissiveStrength) {
            gl.uniform1f(
                program.uniformLocations.emissiveStrength,
                object.emissiveStrength || 0.0
            );
        }
        ...
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, buffers.index);
        // Para objetos emissivos finos (como monitores planos), desabilita cull face
        // para garantir que sejam visíveis mesmo se a câmera estiver "por trás".
        ...
        if (object.isEmissive && wasCullEnabled) {
            gl.enable(gl.CULL_FACE);
        }
    }
