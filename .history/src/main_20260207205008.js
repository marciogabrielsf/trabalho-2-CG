class Application {
    ...
    async loadOBJModels() {
        ...
        try {
            ...
            // Chair center is around X=9.4, Y=11.8, Z=5
            // We need to offset to position properly
            // Target positions (in front of monitors, facing them):
            ...
            for (const pos of chairPositions) {
                // Offset from chair's local origin (X=9.4, Y=11.8, Z=5)
                const chair = this.renderer.addObject(
                    chairGeometry,
                    new Vector3(pos.x - 9.4, -11.8, pos.z - 5),
                    new Vector3(0, 0, 0),
                    new Vector3(1, 1, 1)
                );
