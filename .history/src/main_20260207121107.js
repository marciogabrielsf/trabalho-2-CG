class Application {
    ...
    async setupScene() {
        ...
        const deskPosition = new Vector3(3, 0.5, 3);

        const table = this.renderer.addObject(
            cubeGeometry,
            deskPosition,
            new Vector3(0, 0, 0),
            new Vector3(2.0, 0.2, 1.0)
        );
        table.receiveShadows = true;
        table.emissiveStrength = 0.0;

        const monitorTexture = await this.renderer.loadTexture('./assets/textures/2548700.jpg');

        const monitor = this.renderer.addObject(
            cubeGeometry,
            new Vector3(deskPosition.x, deskPosition.y + 0.6, deskPosition.z - 0.2),
            new Vector3(0, 0, 0),
            new Vector3(0.6, 0.4, 0.05),
            monitorTexture
        );
        monitor.receiveShadows = false;
        monitor.emissiveStrength = 1.0;

        this.renderer.monitorPosition = monitor.position;
