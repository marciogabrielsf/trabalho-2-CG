class Application {
    ...
    async loadOBJModels() {
        ...
        try {
            ...
            // Load textures for materials that have map_Kd
            if (buildingGeometry.materials) {
                console.log('Materials loaded:', buildingGeometry.materials.size);
                
                // Check if any material has textures
                for (const [name, material] of buildingGeometry.materials) {
                    if (material.map_Kd) {
                        console.log(`Material "${name}" has texture: ${material.map_Kd}`);
                        // Load the texture
                        try {
                            ...
                            material.texture = await this.renderer.loadTexture(textureUrl);
                        } catch (error) {
                            console.error(`Failed to load texture for material ${name}:`, error);
                        }
                    }
                }
