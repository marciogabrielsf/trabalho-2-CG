class Application {
    ...
    async loadOBJModels() {
        ...
        try {
            const cadeiraGeometry = await OBJLoader.load('./assets/models/cadeira.obj', true);
            const cadeiraStats = OBJLoader.getStats(cadeiraGeometry);
            console.log('Cadeira loaded:', cadeiraStats);
            
            // Aplica cor verde à cadeira
            if (cadeiraGeometry.positions && cadeiraGeometry.positions.length > 0) {
                const numVertices = cadeiraGeometry.positions.length / 3;
                cadeiraGeometry.colors = new Float32Array(numVertices * 3);
                for (let i = 0; i < numVertices; i++) {
                    cadeiraGeometry.colors[i * 3] = 0.2;     // R
                    cadeiraGeometry.colors[i * 3 + 1] = 0.8; // G (verde)
                    cadeiraGeometry.colors[i * 3 + 2] = 0.3; // B
                }
            }
            
            // Posiciona a cadeira na frente da mesa (Z positivo, rotacionada 180 graus para olhar para a mesa)
            const cadeira = this.renderer.addObject(
                cadeiraGeometry,
                new Vector3(0, 0, 15),        // Posição: frente da mesa
                new Vector3(0, Math.PI, 0),   // Rotação: 180 graus no eixo Y (virada para a mesa)
                new Vector3(1, 1, 1)          // Escala: tamanho normal
            );
            this.objModels.push(cadeira);
        } catch (error) {
            console.error('Failed to load cadeira:', error);
        }
