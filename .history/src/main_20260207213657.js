class Application {
    ...
    async loadOBJModels() {
        ...
        // Load chairs for the 3 monitors
        // Monitors are at approximately X=23, Z=-5.8, -3.7, -7.8
        // Chairs should be placed in front (lower X) facing the monitors
        try {
            const chairGeometry = await OBJLoader.load('./assets/models/cadeira.obj', true);
            const chairStats = OBJLoader.getStats(chairGeometry);
            console.log('Chair loaded:', chairStats);

            if (chairGeometry.materials) {
                OBJLoader.applyMaterialColors(chairGeometry, chairGeometry.materials);
            }

            // Chair dimensions: X(0-0.4), Y(0-0.95), Z(0-0.4)
            // Chair center is at X=0.2, Y=0.475, Z=0.2
            // Target positions (in front of monitors, facing the wall):
            // Monitor 1: X=23.4, Z=-5.8 → Chair at X=20, Z=-5.8
            // Monitor 2: X=23.1, Z=-3.7 → Chair at X=20, Z=-3.7
            // Monitor 3: X=23.5, Z=-7.8 → Chair at X=20, Z=-7.8

            const chairPositions = [
                { x: 20, z: -5.8 },  // Chair for monitor 1
                { x: 20, z: -3.7 },  // Chair for monitor 2
                { x: 20, z: -7.8 }   // Chair for monitor 3
            ];

            for (const pos of chairPositions) {
                // Offset from chair's local center (X=0.2, Y=0.475, Z=0.2)
                // Rotate 180° to face the monitors (encosto virado para trás)
                const chair = this.renderer.addObject(
                    chairGeometry,
                    new Vector3(pos.x - 0.2, -0.475, pos.z - 0.2),
                    new Vector3(0, Math.PI, 0),
                    new Vector3(1, 1, 1)
                );
                this.objModels.push(chair);
            }

            console.log('3 chairs placed in front of monitors');
        } catch (error) {
