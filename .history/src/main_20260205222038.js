class Application {
    ...
    async setupScene() {
        ...
        // Textura para os monitores do laboratório (usar imagem existente como tela)
        const monitorTexture = await this.renderer.loadTexture('./assets/textures/2548700.jpg');
        ...
        // Monitores como superfícies com textura "emissiva"
        const monitorPlane = Plane.createGeometry(1.0, 0.6, 1);

        // Posição base do laboratório (próximo ao ponto de luz pontual)
        // Levemente afastado da parede e um pouco mais alto
        const labBasePos = new Vector3(-5, 1.4, 0.6);

        // Três monitores lado a lado
        const monitorOffsets = [
            new Vector3(-0.8, 0.0, 0.0),
        ...
        for (const offset of monitorOffsets) {
            const pos = new Vector3(
                labBasePos.x + offset.x,
                labBasePos.y + offset.y,
                labBasePos.z + offset.z
            );

            // Plano rotacionado para ficar "em pé" (como tela)
            const monitor = this.renderer.addObject(
                monitorPlane,
                pos,
                // Rotaciona o plano para ficar em pé e levemente voltado para o corredor
                new Vector3(-Math.PI / 2, Math.PI, 0),
                new Vector3(1, 1, 1),
                monitorTexture
            );

            monitor.isEmissive = true;
            monitor.emissiveStrength = 2.0; // brilho extra das telas
        }

        // Marcador visual da luz pontual ("lâmpada" perto dos monitores)
        const lightMarkerGeometry = Cube.createGeometry();
        const lightMarker = this.renderer.addObject(
            lightMarkerGeometry,
            this.renderer.pointLightPosition.clone(),
            new Vector3(0, 0, 0),
            new Vector3(0.2, 0.2, 0.2)
        );
        lightMarker.isEmissive = true;
        lightMarker.emissiveStrength = 3.0;
