class Application {
    ...
    async loadOBJModels() {
        ...
        try {
            ...
            // 3 monitors on desk, rotated -90° Y to face the room center (-X)
            // Y = 3.65 so base (Y=0.34 local) lands on desk surface (Y≈3.99)
            // X = 12 (back of desk section 9.7-12.9)
            // Z = -5, 0, 5 (spread along the desk length)
            const monitorPositions = [
                new Vector3(12, 3.65, -5),
                new Vector3(12, 3.65, 0),
                new Vector3(12, 3.65, 5)
            ];
