class Application {
    ...
    async loadOBJModels() {
        ...
        try {
            ...
            const labPosition = new Vector3(centerX + 5, buildingStats.bounds.min.y + 2, centerZ);
            console.log('Approx. informatics lab position:', labPosition);

            const labMarkerGeometry = Cube.createGeometry();
            const labMarker = this.renderer.addObject(
                labMarkerGeometry,
                labPosition,
                new Vector3(0, 0, 0),
                new Vector3(1.0, 1.0, 1.0)
            );
            this.animatedCubes.push(labMarker);
